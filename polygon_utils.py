...
def angleFromPoints(p1, p2, p3):
    p1p2 = math.sqrt(math.pow(p2[0] - p1[0], 2) + math.pow(p2[1] - p1[1], 2))
    p3p2 = math.sqrt(math.pow(p2[0] - p3[0], 2) + math.pow(p2[1] - p3[1], 2))
    p1p3 = math.sqrt(math.pow(p3[0] - p1[0], 2) + math.pow(p3[1] - p1[1], 2))

    return math.acos((p3p2 * p3p2 + p1p2 * p1p2 - p1p3 * p1p3) / (2 * p3p2 * p1p2))
...
class PolygonSplitter:
    ...
    def findNextEdge(self, edge, ignore_done=True, only_convex=True, ignore_inpoly=False):
        ...
        for a in range(len(v2.edges_out)):
            ...
            angle2 = angleFromPoints(v1, v2, self.vertices[out.v2])

            if math.fabs(angle - angle2) > 0.0001:
                print("Angles differ!")
